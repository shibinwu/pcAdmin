...
</html>  <!--包含对战中心左侧栏文件-->            <div class="centercontent">            <div class="pageheader">            <h1 class="pagetitle">成员编辑</h1>            <span class="pagedesc">这是关于CSGO的世锦赛，保证以下信息的真实性</span>                       <!--  <ul class="hornav">                <li class="current"><a href="#basicform">Basic</a></li>                <li><a href="#validation">Validation</a></li>            </ul> -->        </div><!--pageheader-->                <div id="contentwrapper" class="contentwrapper">        	        	<div id="basicform" class="subcontent">                                                    <div class="contenttitle2">                        <h3>编辑成员</h3>                    </div><!--contenttitle-->                    <form class="stdform" action="<?php echo U('editOk');?>" method="post">                        <p>                            <label>担任职务</label>                            <span class="formwrapper">                                <select data-placeholder="请选择比赛类型" name="role" class="chzn-select" style="width:350px;" tabindex="2">                                    <option value=""></option>                                    <option value="manager" <?php if($data["role"] == manager ): ?>selected="selected"<?php endif; ?>>经理</option>                                    <option value="leader" <?php if($data["role"] == leader ): ?>selected="selected"<?php endif; ?>>领队</option>                                    <option value="coach" <?php if($data["role"] == coach ): ?>selected="selected"<?php endif; ?>>教练</option>                                    <option value="captain" <?php if($data["role"] == captain ): ?>selected="selected"<?php endif; ?>>队长</option>                                    <option value="member" <?php if($data["role"] == member ): ?>selected="selected"<?php endif; ?>>队员</option>                                </select>                            </span>                        </p>                        <p>                            <label>状态</label>                            <span class="formwrapper">                                <select data-placeholder="请选择比赛类型" name="status" class="chzn-select" style="width:350px;" tabindex="2">                                    <option value=""></option>                                    <option value="0" <?php if($data["status"] == 0 ): ?>selected="selected"<?php endif; ?>>等待确认</option>                                    <option value="1" <?php if($data["status"] == 1 ): ?>selected="selected"<?php endif; ?>>正式队员</option>                                    <option value="2" <?php if($data["status"] == 2 ): ?>selected="selected"<?php endif; ?>>拒绝</option>                                </select>                            </span>                        </p>                        <input type="hidden" name="tmid" value="<?php echo ($data["tmid"]); ?>">                        <p>                            <label>修改时间</label>                            <span class="field"><input type="text" name="mtime" value="<?php echo ($data["mtime"]); ?>" class="smallinput" /></span>                            <!-- <small class="desc">Small description of this field.</small> -->                       </p>                        <p class="stdformbutton">                        	<button class="submit radius2" id="btnSubmit">提交</button>                            <input type="reset" class="reset radius2" id="btnReset" value="清空" />                        </p>                                                                    </form>                 </div><!--subcontent--> -->                </div><!--contentwrapper-->        	</div><!-- centercontent -->        </div><!--bodywrapper--></body><script type="text/javascript">    //解决$方法不能用的问题    jQuery.noConflict();    jQuery(document).ready(function($){        //提交或清空方法        $(function(){            $('#btnSubmit').on('click',function(){                $('form').submit();            });            $('#btnReset').on('click',function(){                $('form')[0].reset();            });            $('.exit').click(function () {                window.location.href = "/index.php/Admin/Public/logout";            });            $(".headermenu>li").eq(1).addClass("current").siblings().removeClass("current")        });    });</script></html>