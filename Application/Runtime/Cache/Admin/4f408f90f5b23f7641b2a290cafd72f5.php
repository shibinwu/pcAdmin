...
</html>  <!--包含左侧栏-->            <div class="centercontent">            <div class="pageheader">            <h1 class="pagetitle">添加竞猜队伍</h1>            <span class="pagedesc">操作后台精彩队伍的管理模块，该窗口添加新的竞猜队伍</span>        </div><!--pageheader-->                <div id="contentwrapper" class="contentwrapper">        	        	<div id="basicform" class="subcontent">                    <div class="contenttitle2">                        <h3>竞猜队伍</h3>                    </div><!--contenttitle-->                    <form class="stdform" action="/index.php/Admin/Guessteam/addOk" method="post" enctype="multipart/form-data">                    	                        <p>                        	<label>竞猜队伍名称</label>                            <span class="field"><input type="text" name="gtName" class="smallinput" /></span>                            <!-- <small class="desc">Small description of this field.</small> -->                        </p>                        <!--<p>-->                            <!--<label>队伍类型</label>-->                            <!--<span class="formwrapper">-->                                <!--<select  name="gtType" class="chzn-select" style="width:350px;" tabindex="2">-->                                    <!--<option value="-1">请选择队伍类型</option>-->                                    <!--<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?>-->                                        <!--<?php if($vol["ga_type"] == 1): ?>-->                                            <!--<option value="<?php echo ($vol["ga_id"]); ?>"><?php echo ($vol["ga_name"]); ?></option>-->                                        <!--<?php endif; ?>-->                                    <!--<?php endforeach; endif; else: echo "" ;endif; ?>-->                                <!--</select>-->                            <!--</span>-->                        <!--</p>-->                        <p>                            <label>队伍类型</label>                            <span class="formwrapper">                                <select data-placeholder="请选择比赛类型" name="gtType" class="chzn-select" style="width:350px;" tabindex="2">                                    <option value=""></option>                                    <option value="1">CSGO</option>                                    <option value="2">DOTA2</option>                                    <option value="3">体育赛事</option>                                </select>                            </span>                        </p>                        <p>                            <label>队伍头像</label>                            <span class="field">                                <input type="file" name="gtPic" />                            </span>                        </p>                                                <p class="stdformbutton">                        	<button class="submit radius2">确认提交</button>                            <input type="reset" class="reset radius2" value="清空列表" />                        </p>                    </form>            </div><!--subcontent-->                </div><!--contentwrapper-->        	</div><!-- centercontent --></div><!--bodywrapper--></body><script type="text/javascript">    //解决$方法不能用的问题    jQuery.noConflict();    jQuery(document).ready(function($){        //提交或清空方法        $(function(){            $('#btnSubmit').on('click',function(){                $('form').submit();            });            $('#btnReset').on('click',function(){                $('form')[0].reset();            });            $('.exit').click(function () {                window.location.href = "/index.php/Admin/Public/logout";            });            $(".headermenu>li").eq(3).addClass("current").siblings().removeClass("current")        });    });</script></html>